...
public class PatientListFragment extends ListFragment {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.filter:
    		...
    		d.setContentView(R.layout.patient_filter_choices);
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
    	...
    	change_status.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
					if(patients.getPat_status() == 1)
						changeStatus(patients, schedule);
					if(patients.getPat_status() == 2)
						changeStatus(patients, null);
				return true;
			}
		});
		...
		delete.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			public boolean onMenuItemClick(MenuItem item) {	
				if(patients != null && patients.getPat_status() == 1)
					deletePatients(patients, schedule);
				else
					deletePatients(patients, null);
				return true;
			}
		});
	...
	public void changeStatus(final Patients p, final Schedule s)
	{
		...
		build.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if(s != null && p.getPat_status() == 1)
				{
						Patients patients = p;
						patients.setPat_status(2);
						help.update(patients);
						help.delete(s);
				...
				else
				{
						...
						add_rounds_setup.setOnClickListener(new OnClickListener() {
							public void onClick(View v) {
								...
								schedule.setPatient_id(patient.getId());
	...
	public void updateRounds(final Patients p, final Schedule s)
	{
		Toast.makeText(getActivity(), p.getId()+"/n"+p.getHosp_name()+" - "+p.getHosp_room(), Toast.LENGTH_LONG).show();
		Toast.makeText(getActivity(), s.getLocation()+" @ "+s.getTime(), Toast.LENGTH_LONG).show();
		final Dialog in_d = new Dialog(getActivity());
	...
	public void editPatients(final Patients patient)
	{
		final Dialog d = new Dialog(getActivity());
	...
	public void deletePatients(final Patients p, final Schedule s)
	{
		AlertDialog.Builder build = new AlertDialog.Builder(getActivity());
	...
	public void loadPatientList(int status)
	{

		ArrayList<Patients> master_list = new ArrayList<Patients>();
		ArrayList<Patients> filtered = new ArrayList<Patients>();
		filtered.clear();
		master_list = convertListToPatients(help.getData(this));
	...
	public ArrayList<Patients> convertListToPatients(ArrayList<Object> o)
	{
	...
	public void sortPatients(ArrayList<Patients> p)
	{
	...
	public ArrayList<Patients> filterPatientList(ArrayList<Patients> list, int status)
	{
	...
	public View setHeader(int status)
	{
