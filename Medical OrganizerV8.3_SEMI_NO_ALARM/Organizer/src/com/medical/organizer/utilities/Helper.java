...
public class Helper{
	...
	public final static String ACTION_REPEATING = "com.medical.organizer.utilities.NotifyRecieverRepeating";
	private static NotifyReciever RECIEVER = new NotifyReciever();
	private static NotifyRecieverRepeating RECIEVER_REPEATING = new NotifyRecieverRepeating();
	...
	/*public static void scheduleAlarm(Context context,Schedule s,int rCode, long triggerTime)
	{
		Log.d("ItemCHeck", "Type: "+s.getType());
		Log.d("ItemCHeck", "TIme: "+Schedule.formatDate(s.getTime(), "hh:mm a"));
		...
		try {
			serializedObject = Serializer.serialize(s);
		} catch (IOException e) {
			e.printStackTrace();
		}
		intent.putExtra("schedule", serializedObject);
		intent.setFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
		NotifyAlarm.INTENT = intent;
		NotifyAlarm.startAlarm(rCode);
	}
	
	public static void scheduleRepeatingAlarm(Context context,Object o ,int rCode, long triggerTime)
	{
		Schedule sp = new Schedule();
		String serializePatients = null;
		if(o instanceof Patients)
		{
			Patients p = (Patients) o;
				try {
					serializePatients = Serializer.serialize(p);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			sp.setType("rounds");
			sp.setTitle("Patient Rounds");
			sp.setDescription("Medical Rounds: \n"+p.getMed_history());
			sp.setSchedule_id(p.getSchedule_id());
			sp.setPatient_id(p.getPatient_id());
			sp.setAlarm(true);
			sp.setDate("everyday");
			sp.setLocation(p.getHosp_name()+" - "+p.getHosp_room());
			sp.setDone(false);
			sp.setTime(p.getTime());
			sp.setRequestCode(p.getRequestCode());	
		}
		else
		{
			sp = (Schedule) o;
		}
		Log.d("ItemCHeck", "Type: "+sp.getType());
		Log.d("ItemCHeck", "TIme: "+Schedule.formatDate(sp.getTime(), "hh:mm a"));
		RECIEVER = new NotifyReciever();
		NotifyAlarm.TRIGGER_TIME = triggerTime;
		NotifyAlarm.CONTEXT = context;
		
		IntentFilter inFil = new IntentFilter(ACTION_REPEATING);
		NotifyAlarm.CONTEXT.registerReceiver(RECIEVER_REPEATING, inFil);
		Intent intent = new Intent(ACTION_REPEATING);

		String serializedObject = null;
		try {
			serializedObject = Serializer.serialize(sp);
		} catch (IOException e) {
			e.printStackTrace();
		}
		intent.putExtra("schedule", serializedObject);
		intent.putExtra("patients", serializePatients);
		intent.putExtra("isRounds", true);
		intent.setFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
		NotifyAlarm.INTENT = intent;
		NotifyAlarm.startAlarmRepeating(rCode, Helper.TWENTY_FIVE_SEC);
	}
	*/
	/*public static void cancelRepeatingAlarm(Context context, int requestCode)
	{
		RECIEVER = new NotifyReciever();
		NotifyAlarm.CONTEXT = context;
		...
		Intent intent = new Intent(ACTION_REPEATING);
		NotifyAlarm.INTENT = intent;
		context.unregisterReceiver(RECIEVER_REPEATING);
