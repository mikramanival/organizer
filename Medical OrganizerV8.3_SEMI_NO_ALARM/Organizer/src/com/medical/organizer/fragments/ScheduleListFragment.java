			...
			if(schedule.isAlarm())
			{
				...
				alarmOff.setOnMenuItemClickListener(new OnMenuItemClickListener() {
					public boolean onMenuItemClick(MenuItem item) {
							...
							{
								s.setDone(true);
								//Helper.cancelAlarm(getActivity(), s.getRequestCode());
							}
			...
			public void onClick(DialogInterface dialog, int which) {
				String type = schedule.getType();
					if(schedule.getType().equals("rounds"))
					{
						...
						p.setPat_status(2);
						//Helper.cancelRepeatingAlarm(getActivity(), schedule.getRequestCode());
					...
					{
						if(schedule.isAlarm())
							//Helper.cancelAlarm(getActivity(), schedule.getRequestCode());
						help.delete(schedule);
					}
			...
			public void onClick(View v) {
				...
				build.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						//Helper.scheduleRepeatingAlarm(getActivity(), patient, patient.getRequestCode(), patient.getTime());
