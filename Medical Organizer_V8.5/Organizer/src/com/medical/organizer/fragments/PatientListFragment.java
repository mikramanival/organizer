...
public class PatientListFragment extends ListFragment implements TextWatcher{
								...
								if(help.checkInputs(views))
								{
									...
									build.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
										public void onClick(DialogInterface dialog, int which) {
											...
											schedule.setRequestCode(Helper.generateRequestCode(timeInMills));						
											help.update(patient, Helper.CHANGE_PATIENT_STATUS);
											help.insert(schedule);
											help.scheduleRepeatingAlarm(getActivity(), schedule.getSchedule_id(), schedule.getRequestCode(), schedule.getTime());
											in_d.cancel();
											loadPatientMasterList();
											loadPatientList(1);
											Toast.makeText(getActivity(), "Patient's Status Changed!", Toast.LENGTH_SHORT).show();
			...
			public void onClick(View v) {
				...
				build.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						help.cancelAlarm(getActivity(), patients.getRequestCode());
						help.scheduleRepeatingAlarm(getActivity(), patients.getSchedule_id(), patients.getRequestCode(), patients.getTime());
						loadPatientMasterList();
						loadPatientList(1);
						in_d.cancel();
					}
	...
	private void loadPatientMasterList()
	{
		...
		if(master_list.size() <= 0)
			Toast.makeText(getActivity(), "Patient Records Empty", Toast.LENGTH_LONG).show();
	}
